...
void UnloadModule( OS_ModuleRef inModule, bool inOnlyResourceAccess )
{
	if( inModule != NULL )
	{
		...
		if( inOnlyResourceAccess )
		{
			...
			{
				XMP_Throw("OS_Utils_Android::UnloadModule called with invalid module handle", kXMPErr_InternalFailure);
			}
		}
		else
		{
	...
	OS_ModuleRef inOSModule)
{
	...
	if( inOSModule != NULL )
	{
		...
		{
			XMP_Throw("OS_Utils_Android::GetModulePath: Module handle is present in both global maps", kXMPErr_InternalFailure);
		}
		if (iter != sMapModuleRefToPath.end())
		{
			result = iter->second;
		...
		{
			XMP_Throw("OS_Utils_Android::GetModulePath: Failed to find inOSModule in global map !", kXMPErr_InternalFailure);
		}
	}
